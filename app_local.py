"""
app_local.py — Interface locale du système RAG hybride.
...
WEIGHTS_PATH = PROJECT_ROOT / "models" / "custom_pooler.pt"
FAISS_PATH = PROJECT_ROOT / "index" / "squad_extended.faiss"
CORPUS_PATH = PROJECT_ROOT / "index" / "corpus_extended.pkl"
...
# CSS compact pour tout faire tenir sur 1 page
CUSTOM_CSS = """
<style>
    /* Réduire drastiquement le padding global */
    .block-container {
    ...
    /* Réduire les hr (dividers) */
    hr {
        margin: 0.6rem 0 !important;
    }

    /* Cadre de réponse compact */
    .answer-box {
        background: #f8f7ff;
    ...
    /* Banner d'info compact mais détaillé */
    .info-banner {
        background: #eff6ff;
...
@st.cache_resource
def load_engine():
    """Charge tous les composants RAG et assemble le système."""
    missing = []
    for name, path in [
        ("custom_pooler.pt", WEIGHTS_PATH),
        ("squad_extended.faiss", FAISS_PATH),
        ("corpus_extended.pkl", CORPUS_PATH),
    ...
    dense_engine.load(str(FAISS_PATH), str(CORPUS_PATH))
...
with st.spinner("Chargement du modèle et des index..."):
    system = load_engine()


# Exemples de questions (chips compacts)
st.markdown(
...
if query and (search_clicked or st.session_state.current_query == query):
    ...
    # Sources retenues après reranking
